...
class SppagebuilderAddonArticles extends SppagebuilderAddons {
    ...
    public function render() {
        ...
        //New added
        $variation = (isset($this->addon->settings->variation)) ? $this->addon->settings->variation : '';

        $items = SppagebuilderHelperArticles::getArticles($limit, $ordering, $catid, TRUE, $post_type);
        $output = '';

        if (count($items)) {
            $output = '<div class="sppb-addon sppb-addon-articles ' . $class . ' ' . $variation . '">';
                ...
                if (!$hide_thumbnail) {
                    if ($item->post_format == 'gallery') {
                        ...
                        if (isset($item->image_thumbnail) && $item->image_thumbnail) {
                            $output .= '<a href="' . $item->link . '" class="sppb-img-wrapper" itemprop="url"><img class="sppb-img-responsive" src="' . $item->image_thumbnail . '" alt="' . $item->title . '" itemprop="thumbnailUrl"></a>';
                        }
                    }
                }
            ...
            if ($link_articles) {
                ...
                $output .= '<a href="' . JRoute::_(ContentHelperRoute::getCategoryRoute($catid)) . '" id="btn-' . $this->addon->id . '" class="sppb-see-all-btn sppb-btn' . $all_articles_btn_class . '">' . $all_articles_btn_text . '</a>';
            }
