...
class SppagebuilderAddonCarousel extends SppagebuilderAddons {
	...
	public function render() {
		...
		$output  = '<div id="sppb-carousel-'. $this->addon->id .'" data-interval="'.$interval.'" class="sppb-carousel sppb-slide' . $class . ' ' . $variation . '"' . $carousel_autoplay .'>';
		...
		foreach ($this->addon->settings->sp_carousel_item as $key => $value) {
			$main_bg;
			if(is_object($value->bg)){
				$main_bg = $value->bg->src;
			} else {
				$main_bg = $value->bg;
			}
			$button_url = (isset($value->button_url) && $value->button_url) ? $value->button_url : '';

			$output   .= '<div class="sppb-item sppb-item-'. $this->addon->id . $key . ' ' . ((isset($main_bg) && $main_bg) ? ' sppb-item-has-bg' : '') . (($key == 0) ? ' active' : '') .'">';
			$output  .= (isset($main_bg) && $main_bg) ? '<img src="' . $main_bg . '" alt="' . $value->title . '">' : '';
	...
	public static function getTemplate(){
		...
		<div class="sppb-carousel sppb-slide {{ data.class }} {{variation}}" id="sppb-carousel-{{ data.id }}" data-interval="{{ interval }}" {{{ autoplay }}}>
			<# if(data.controllers){ #>
				...
				<# _.each(data.sp_carousel_item, function (carousel_item, key){ #>
					<#
						...
						if((typeof carousel_item.bg !== "undefined") && (typeof carousel_item.bg.src !== "undefined")){
							main_bg = carousel_item.bg.src;
						} else {
							main_bg = carousel_item.bg
						}
						#>
					<div class="sppb-item {{ classNames }}">
						<img src=\'{{ main_bg }}\' alt="{{ carousel_item.title }}">
