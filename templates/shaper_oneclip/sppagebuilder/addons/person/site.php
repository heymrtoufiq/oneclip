...
class SppagebuilderAddonPerson extends SppagebuilderAddons{
	...
	public function render() {
		...
		if($facebook || $twitter || $google_plus || $youtube || $linkedin || $pinterest || $flickr || $dribbble || $behance || $instagram) {
			...
			$output  .= '<div class="sppb-addon sppb-addon-person ' . $alignment . ' ' . $class . ' ' . $variation . ' social-' . $social_position . '">';
			...
			if($name || $designation || $email) {
				...
				if($name) $output  .= '<h5 class="sppb-person-name">' . $name . '</h5>';
				if($designation) $output  .= '<p class="sppb-person-designation">' . $designation . '</p>';
				if($email) $output  .= '<p class="sppb-person-email">' . $email . '</p>';
				$output  .= '</div>';
			}
	...
	public static function getTemplate(){
		...
		$output ='
			...
			#>
			<div class="sppb-addon sppb-addon-person {{ data.alignment }} {{ data.class}} {{variation}} social-{{data.social_position}}">
				<div class="sppb-addon-content">
				<# if(!_.isEmpty(data.image)) { #>
					<div class="sppb-person-image">
						<img class="sppb-img-responsive" src=\'{{ main_image }}\' alt="{{ data.name }}">
				...
				<# if(data.name || data.designation || data.email ){ #>
					<div class="sppb-person-information">
						<# if(!_.isEmpty(data.name)) { #>
							<h5 class="sppb-person-name">{{ data.name}}</h5>
						<# } #>
						<# if(!_.isEmpty(data.designation)) { #>
							<p class="sppb-person-designation">{{ data.designation}}</p>
						<# } #>
						<# if(!_.isEmpty(data.email)) { #>
							<p class="sppb-person-email">{{ data.email}}</p>
						<# } #>
					</div>
				<# } #>
